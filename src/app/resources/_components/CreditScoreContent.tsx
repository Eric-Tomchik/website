...
const einCreditCards: CreditCardData[] = [
    ...
    <div className="space-y-20">
      ...
      <div className="space-y-4">
        ...
        <p className="text-lg text-surface-300 leading-relaxed max-w-3xl">
          This is the free companion resource referenced throughout the book. Vendor reporting
          relationships, card requirements, and bureau thresholds change — this page is updated
          whenever significant changes are confirmed.
        </p>
      ...
      <section className="space-y-10">
        <SectionHeader
          ...
          description="Vendors offering Net-30 terms with credit bureau reporting, organized by which bureaus they report to."
          id="cs-vendors"
        />
        <div className="space-y-10">
          <div className="card p-6"><VendorTable vendors={vendorsAllThree} bureaus="All Three Bureaus (D&B + Experian + Equifax)" /></div>
          <div className="card p-6"><VendorTable vendors={vendorsDnbExperian} bureaus="D&B and Experian" /></div>
          <div className="card p-6"><VendorTable vendors={vendorsDnbOnly} bureaus="D&B Only" /></div>
          <div className="card p-6"><VendorTable vendors={vendorsExperianEquifax} bureaus="Experian and Equifax" /></div>
        </div>
      </section>
      ...
      <section className="space-y-8">
        <SectionHeader
          ...
          description="Business credit cards available without a personal credit check or personal guarantee."
          id="cs-credit-cards"
        />
              ...
              <tbody className="divide-y divide-surface-800">
                {einCreditCards.map((card) => (
                  <tr key={card.name} className="hover:bg-surface-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4"><span className={card.pg === 'No' ? 'text-green-400' : 'text-yellow-400'}>{card.pg}</span></td>
                    <td className="py-3 px-4"><span className={card.personalCheck === 'No' ? 'text-green-400' : 'text-yellow-400'}>{card.personalCheck}</span></td>
      ...
      <section className="space-y-8">
        <SectionHeader
          ...
          description="Complete reference for every major business credit score from all three bureaus and FICO SBSS."
          id="cs-score-ranges"
        />
        <div className="space-y-8">
          <div className="card p-6"><ScoreTable scores={dnbScores} bureau="Dun & Bradstreet" /></div>
          <div className="card p-6"><ScoreTable scores={experianScores} bureau="Experian Business" /></div>
          <div className="card p-6"><ScoreTable scores={equifaxScores} bureau="Equifax Business" /></div>
          <div className="card p-6"><ScoreTable scores={ficoSbss} bureau="FICO SBSS" /></div>
        </div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {governmentResources.map((resource) => (
            <a key={resource.name} href={resource.url} target="_blank" rel="noopener noreferrer" className="card p-5 group hover:border-emerald-500/40 transition-all">
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1.5">
                  <h3 className="text-white font-semibold group-hover:text-emerald-400 transition-colors">{resource.name}</h3>
                  <p className="text-sm text-surface-400">{resource.purpose}</p>
                  <p className="text-xs text-emerald-400/70 font-mono">{resource.urlLabel}</p>
                </div>
                <ExternalLink className="w-4 h-4 text-surface-500 group-hover:text-emerald-400 transition-colors flex-shrink-0 mt-1" />
              </div>
            </a>
          ))}
        </div>
      ...
      <section className="space-y-8">
        <SectionHeader icon={Search} title="Credit Bureaus & Monitoring" description="The three business credit bureaus and monitoring services to track your progress." id="cs-bureaus" />
        <div className="space-y-8">
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-white">Business Credit Bureaus</h3>
            <div className="grid sm:grid-cols-3 gap-4">
              {creditBureaus.map((bureau) => (
                <a key={bureau.name} href={bureau.url} target="_blank" rel="noopener noreferrer" className="card p-5 group hover:border-emerald-500/40 transition-all">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <h4 className="text-white font-semibold group-hover:text-emerald-400 transition-colors">{bureau.name}</h4>
                      <ExternalLink className="w-3.5 h-3.5 text-surface-500 group-hover:text-emerald-400 transition-colors" />
                    </div>
                    <p className="text-sm text-surface-400">{bureau.functions}</p>
                    <p className="text-xs text-emerald-400/70 font-mono">{bureau.urlLabel}</p>
                  </div>
                </a>
              ))}
            </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm text-surface-300 space-y-2">
                <p>Bureau reporting relationships can change at any time. Always verify current reporting status directly with the vendor before opening an account.</p>
                <p>Credit card terms, requirements, and rewards programs are subject to change. Verify all details directly with the issuer before applying.</p>
                <p>This page is for informational and educational purposes only and does not constitute legal, financial, or tax advice.</p>
              </div>
            </div>
          </div>
        </div>
