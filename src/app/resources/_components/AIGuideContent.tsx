...
const categories: Category[] = [
  ...
  {
    name: 'Image Generation', id: 'image',
    platforms: [
  ...
  {
    name: 'Video Generation', id: 'video',
    platforms: [
  ...
  {
    name: 'Audio, Music & Voice AI', id: 'audio',
    platforms: [
  ...
  {
    name: 'Coding & Development AI', id: 'coding',
    platforms: [
  ...
  {
    name: 'Writing & Content AI', id: 'writing',
    platforms: [
  ...
  {
    name: 'Design & Creative AI', id: 'design',
    platforms: [
  ...
  {
    name: 'Productivity & Workspace AI', id: 'productivity',
    platforms: [
  ...
  {
    name: 'Marketing, Sales & CRM AI', id: 'marketing',
    platforms: [
  ...
  {
    name: 'Data, Analytics & BI', id: 'analytics',
    platforms: [
  ...
  {
    name: 'Customer Service & Support AI', id: 'support',
    platforms: [
  ...
  {
    name: 'Automation & Workflow AI', id: 'automation',
    platforms: [
  ...
  {
    name: 'Industry-Specific AI', id: 'industry',
    platforms: [
  ...
  {
    name: 'Open Source & Self-Hosted AI', id: 'open-source',
    platforms: [
...
interface StackItem { need: string; tool: string; cost: string; }
interface Stack { name: string; range: string; items: StackItem[]; }

const stacks: Stack[] = [
  { name: 'Starter Stack', range: 'Free – $20/mo', items: [
  ...
  { name: 'Professional Stack', range: '$40 – $60/mo', items: [
  ...
  { name: 'Developer Stack', range: '$30 – $50/mo', items: [
  ...
  { name: 'Creator Stack', range: '$50 – $80/mo', items: [
  ...
  { name: 'Enterprise Stack', range: '$50 – $100+/user/mo', items: [
...
function SectionHeader({ icon: Icon, title, description, id }: { icon: React.ComponentType<{ className?: string }>; title: string; description: string; id: string }) {
  return (
...
export default function AIGuideContent() {
  const totalFree = categories.reduce((acc, c) => acc + c.platforms.filter((p) => p.free).length, 0);

  return (
    <div className="space-y-20">
      {/* Intro */}
      <div className="space-y-4">
        <h2 className="text-3xl sm:text-4xl font-extrabold text-white">The Complete AI Platform Guide</h2>
        <p className="text-lg text-surface-300 leading-relaxed max-w-3xl">
          AI platforms ship updates weekly, change pricing monthly, and launch entirely new products quarterly. This companion page keeps all 153 platform URLs, pricing tiers, and recommended stacks current.
        </p>
      ...
      <div className="card p-6 space-y-3">
        <p className="text-xs text-surface-500 uppercase tracking-wider font-semibold">Platform Categories</p>
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <a key={cat.id} href={`#ai-cat-${cat.id}`} className="text-xs px-2.5 py-1 rounded-full bg-surface-800/60 text-surface-300 border border-surface-700/50 hover:border-violet-500/40 hover:text-white transition-all">
              {cat.name}
            </a>
          ))}
        </div>
      ...
      <section className="space-y-12">
        <SectionHeader icon={Globe} title="Platform Directory" description="All 153 platforms from the book, organized by category. Each entry includes the live URL, parent company, free tier availability, and starting price." id="ai-directory" />
        {categories.map((category) => (
          <div key={category.id} id={`ai-cat-${category.id}`} className="scroll-mt-32 space-y-4">
            <h3 className="text-xl font-bold text-white border-b border-surface-800 pb-3">
              {category.name}
              <span className="text-sm font-normal text-surface-400 ml-3">{category.platforms.length} platforms</span>
            </h3>
                    ...
                    {category.platforms.map((p) => (
                      <tr key={p.name} className="hover:bg-surface-800/40 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <a href={`https://${p.url}`} target="_blank" rel="noopener noreferrer" className="text-brand-400 hover:text-brand-300 transition-colors text-xs font-mono inline-flex items-center gap-1">
                            {p.url}<ExternalLink className="w-3 h-3 flex-shrink-0" />
                          </a>
                        </td>
                        <td className="py-3 px-4 text-surface-300">{p.company}</td>
                        <td className="py-3 px-4 text-center">{p.free ? <span className="text-green-400 text-xs font-semibold">✓</span> : <span className="text-surface-500 text-xs">—</span>}</td>
      ...
      <section className="space-y-10">
        <SectionHeader icon={Layers} title="Recommended Stacks" description="Pre-built tool combinations for different budgets and use cases." id="ai-stacks" />
              ...
              <div className="space-y-3">
                {stack.items.map((item) => (
                  <div key={item.need} className="flex items-start justify-between gap-3">
                    ...
                    <span className="text-xs text-surface-300 font-mono whitespace-nowrap flex-shrink-0 mt-3">{item.cost}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="space-y-8">
        <SectionHeader icon={DollarSign} title="Pricing Overview" description="Quick stats across all 153 platforms." id="ai-pricing" />
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Platforms Tracked', value: '153', sub: 'Across 14 categories' },
            { label: 'Offer Free Tier', value: `${totalFree}`, sub: 'Of 153 platforms' },
          ...
          <div className="flex gap-4">
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm text-surface-300 space-y-2">
                <p>AI platform pricing, features, and availability change frequently. Prices shown were verified as of {LAST_UPDATED}. Always check the platform&apos;s website for current pricing.</p>
                <p>Free tiers may have usage limits, feature restrictions, or data-handling policies that differ from paid plans. Review each platform&apos;s terms of service and privacy policy before uploading sensitive data.</p>
              </div>
            </div>
          </div>
        </div>
